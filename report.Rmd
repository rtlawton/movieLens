...
where $n_i$ is the number of ratings of movie *i*, $r_{i,u}$ is the rating of movie *i* by user *u* and $\mu$ is the global mean of ratings. The sum is taken over all ratings of movie *i*.
...
On the basis of this plot, $\lambda_m$ was set at `r lambda_m`. For the movie effect the benefit of regularising with the parameter $\lambda_m$ is fairly small. The estimated RMSE (from our test hold-out set) was `r est_rmse_m_0` with no regularisation ($\lambda_m$ = 0). This dropped to `r est_rmse_m_best` when $\lambda_m$ was set to `r lambda_m`. The corresponding indexed vector $b_i$ for $\lambda_m$ = `r lambda_m` was saved as "movie effect", for future testing on the *validation* set, and a column "$b_i$" added to the *edx* dataframe.
\
...
The RMSE of `r round(rmse_4,4)` implies that our predictions are likely to lie within $\pm$ `r 2*round(rmse_4,1)` of the actual value. On a possible rating range of 0-5 this is not particularly impressive, on the face of it.
...
A second, more intractable problem is that movie ratings involve a large measure of subjectivity. The residual noise $\epsilon_{i,u}$ will therefore include a 'halo effect': how do extraneous circumstances affect a raters score? How reproducible is an individual rating? (How differently would a user rate the same movie on a different day?) It is unlikely that an individual rating would have a reproducibility much better than $\pm$ 1.
